...
| D2 | Sync architecture: **suspend & append** (strategy D). No submit-time replay of user commands. hal CBs are left open at table-relevant boundaries; exact barriers + metadata writes are appended at submit, then closed. Realized via recorded insertion points + submit-time splice (`close_and_insert_at`), generalizing the deferred query-resolve mechanism — hal CBs are not literally kept open and hal needs no open-tail contract; see plans/m0-notes.md (2026-07-09). |
...
- 2026-07-09 — M0 wave 1 landed: 0.1 (feature bits at FeaturesWGPU word-0 bits 14/16/25 — the "second u64" note was stale) and 0.2 (getResource only; hasResource + samplers deferred to M1 with metadata; index u32-only). D2 realization updated to insertion-point splice (approved, see m0-notes.md).
- 2026-07-09 — M0 wave 2 landed: 0.3 naga SPIR-V unchecked lowering (aliased runtime-array globals, unconditional NonUniform, spirv-val confirms D16 incl. vulkan1.2 env; spv≥1.4 interface listing handled) and 0.4 hal trait surface (create/destroy/update_table_slot + set_resource_table, uses_resource_table on PipelineLayoutDescriptor, dyn layer, noop impl, backend stubs). See plans/m0-notes.md.
- 2026-07-09 — M0 wave 3 landed: 0.5 Vulkan hal implementation (shared UAB set layout, per-table variable-count pool/set, pipeline-layout splice at index = group count, deferred naga compile for table shaders; windowless smoke example PASSES on real GPU with zero VVL output — D16 runtime-confirmed) and 0.6 core ResourceTable object (registry/ids, snatchable hal object, QuerySet-style deferred destroy, per-slot fence gating scaffold, trace/player, 8 new tests). See plans/m0-notes.md.
- 2026-07-09 — M0 wave 4 landed: 0.7 wgpu-core encoder state + pipeline plumbing (pipeline-layout uses_resource_table bool + validation; compute-pass set_resource_table command + render-pass-descriptor table binding; lazy dirty-flag hal emission; StatelessTracker recording; trace/player armed; 103 validation tests green, +9 new). User decisions: feature-name strings stay per naming convention ("wgpu-sampling-resource-table" etc.); M0 rejects table-using render bundles with validation error (0.8 implements). See plans/m0-notes.md.
- 2026-07-09 — M0 wave 5 landed: 0.8 queue machinery (D2 two-phase splice, host slot updates, submission marking + teardown deferral, M0 bundle rejection), 0.11 public API incl. slot updates + as_hal, first e2e GPU smoke passing on 3 adapters VVL-clean; interim fresh-eyes review over waves 1–5 found 2 critical + 1 medium (all fixed same day) and de-risked the naga interface over-listing via spirv-val. See plans/m0-notes.md.
- Wave 6 (e2e suite): 97eb3903 — 19 new tests across regression/render/lifecycle/negative/features modules; 60/60 on 3 adapters, VVL-clean; no product bugs found.
- Wave 7 (0.9 conflict validation + 0.10 dirty-bit barriers): a2269e51, 42040e57, 8a2816c9 — submit-time writable-member rejection; compute-pass hazard barrier scheme with new hal `resource_table_memory_barrier`; 6 new e2e + 3 validation tests; 69/69 on 3 adapters, VVL-clean.
- Wave 8 (final review + fixes + docs): ce7c4a48, 34356d41, a5566b31 — fresh-eyes review found 1 MEDIUM (layout-safety, fixed + independently verified) and 1 LOW (perf gate, fixed); CHANGELOG + doc notes landed; residual sandwich-layout hole documented and deferred to D11; suite at 93/93 e2e ×3 adapters, 120 validation.
