...
pub(super) fn encode_render_pass(
    ...
    let (mut scope, pending_discard_init_fixups, mut pending_query_resets) = {
        ...
        // The command stream has been fully drained above, leaving
        // `base.commands` empty but still holding its grown capacity.
        // `base.dynamic_offsets`, in contrast, is consumed by reference (each
