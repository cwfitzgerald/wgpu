...
/// Update After Bind is used on intel only, and all intel devices have at least 524288
const MAX_UPDATE_AFTER_BIND_DESCRIPTORS_IN_ALL_POOLS: u32 = 524288;
                ...
                Some(
                    vk::PhysicalDeviceVulkan12Features::builder()
                        ...
                        .descriptor_binding_sampled_image_update_after_bind(
                            private_caps.update_after_bind,
                        )
...
impl PhysicalDeviceCapabilities {
    ...
    fn to_wgpu_limits(&self, private_caps: &super::PrivateCapabilities) -> wgt::Limits {
        let limits = &self.properties.limits;
        let max_sampled_textures = if private_caps.update_after_bind {
        ...
        let max_storage_textures = if private_caps.update_after_bind {
            if let Some(di) = self.descriptor_indexing {
        ...
        let max_uniform_buffers = if private_caps.update_after_bind {
            if let Some(di) = self.descriptor_indexing {
        ...
        let max_storage_buffers = if private_caps.update_after_bind {
            if let Some(di) = self.descriptor_indexing {
...
impl super::Instance {
    pub fn expose_adapter(
        ...
        let private_caps = super::PrivateCapabilities {
            ...
            update_after_bind: {
                if phd_capabilities.properties.vendor_id == db::intel::VENDOR {
                    if let Some(ref di) = phd_features.descriptor_indexing {
                    ...
                    } else if let Some(di) = phd_features.vulkan_1_2 {
                        di.descriptor_binding_sampled_image_update_after_bind != 0
                            && di.descriptor_binding_storage_image_update_after_bind != 0
                            && di.descriptor_binding_uniform_buffer_update_after_bind != 0
                            && di.descriptor_binding_storage_buffer_update_after_bind != 0
        ...
        let capabilities = crate::Capabilities {
            limits: phd_capabilities.to_wgpu_limits(&private_caps),
