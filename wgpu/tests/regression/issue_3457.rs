...
fn pass_reset_vertex_buffer() {
    initialize_test(TestParameters::default(), |ctx| {
        ...
        let mut double_rpass = encoder.begin_render_pass(&RenderPassDescriptor {
        ...
        double_rpass.set_vertex_buffer(0, vertex_buffer.slice(..));
        double_rpass.set_vertex_buffer(1, vertex_buffer.slice(..));
        double_rpass.draw(0..3, 0..1);

        drop(double_rpass);

        let mut single_rpass = encoder.begin_render_pass(&RenderPassDescriptor {
        ...
        single_rpass.set_vertex_buffer(0, vertex_buffer.slice(..));
